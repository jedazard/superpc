superpc.predictionplot <- function (train.obj, 
                                    data, 
                                    data.test,  
                                    threshold, 
                                    n.components=3, 
    ...
    if (is.null(shrinkage)) { 
        ...
        fit.cts <- superpc.predict.red(train.obj, 
                                       data, 
                                       data.test, 
                                       threshold=threshold, 
                                       n.components=n.components, 
    ...
    if (train.obj$type == "survival") {
        if (n.components == 1) {
            layout(matrix(c(1,2),1,2, byrow=TRUE), widths=c(.80,.20), heights=c(1,1))
        }
        if (n.components == 2) {
            layout(matrix(c(1,4,2,5,3,6),3,2, byrow=TRUE), widths=c(.80,.20), heights=c(.34,.33,.33))
        }
        if (n.components == 3) {
            layout(matrix(c(1,5,2,6,3,7,4,8),4,2, byrow=TRUE), widths=c(.80,.20), heights=rep(.25,4))
        }

        if (is.null(shrinkage)) {
            fit.groups<- superpc.predict(train.obj, 
                                         data, 
                                         data.test, 
                                         threshold=threshold, 
                                         n.components=n.components, 
            ...
            fit.groups <- superpc.predict.red(train.obj, 
                                              data, 
                                              data.test, 
                                              threshold=threshold, 
                                              n.components=n.components, 
        ...
        for (i in 1:n.components) {
            if (is.null(shrinkage)) {
                plot(survival::survfit(formula=survival::Surv(data.test$y,data.test$censoring.status) ~ fit.groups$v.pred[,i]), col=2:lastc, xlab="time", ylab="Prob survival", xlim=c(0,xmax))
            } else {
                plot(survival::survfit(formula=survival::Surv(data.test$y,data.test$censoring.status) ~ fit.groups$v.test[1,,i]), col=2:lastc, xlab="time", ylab="Prob survival", xlim=c(0,xmax))
            }
        ...
        if (n.components > 1) {
            if (is.null(shrinkage)) {
                plot(survival::survfit(formula=survival::Surv(data.test$y,data.test$censoring.status) ~ fit.groups$v.pred.1df), col=2:lastc, xlab="time", ylab="Prob survival" , xlim=c(0,xmax))
            } else {
                plot(survival::survfit(formula=survival::Surv(data.test$y,data.test$censoring.status) ~ fit.groups$v.test.1df[1,]), col=2:lastc, xlab="time", ylab="Prob survival" , xlim=c(0,xmax))
            }
