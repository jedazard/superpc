superpc.cv <- function (fit, 
    ...
    for (j in first:last) {
        ...
        data.temp <- list(x=data$x[, -folds[[j]]], 
                         y=data$y[-folds[[j]]], 
            ...
            if (sum(cur.features) > 1) {
                ...
                if (compute.fullcv) {
                    for (k in 1:ncol(cur.v)) {
                        if (type == "survival") {
                            junk <- survival::coxph(formula=survival::Surv(data$y[folds[[j]]], data$censoring.status[folds[[j]]]) ~ cur.v[, 1:k], 
                                                    control=survival::coxph.control(iter.max=10))$loglik
            ...
            for (i in 1:n.threshold) {
                for (j in 1:n.components) {
                    if (sum(is.na(v.preval[, 1:j, i])) == 0) {
                        if (type == "survival") {
                            junk <- survival::coxph(formula=survival::Surv(data$y, data$censoring.status) ~ v.preval[, 1:j, i])$loglik
